...
from . import bajas, consentimiento, encuestas, paneles, personas, remoto, revision
...
def alta_panelista(ctx, actor, params, cuerpo, consulta):
    resultado = personas.alta(ctx.local, cuerpo, actor=actor.uid)
    ctx.local.commit()
...
def ficha_panelista(ctx, actor, params, cuerpo, consulta):
    return 200, personas.ficha(ctx.local, params["id_persona"])
...
def listar_revisiones(ctx, actor, params, cuerpo, consulta):
    return 200, {"items": revision.listar(ctx.local, consulta.get("estado", "pendiente"))}
...
def crear_panel(ctx, actor, params, cuerpo, consulta):
    resultado = paneles.crear(ctx.local, cuerpo.get("nombre"), cuerpo.get("descripcion"))
    ctx.local.commit()
...
def listar_paneles(ctx, actor, params, cuerpo, consulta):
    return 200, {"items": paneles.listar(ctx.local)}


@ruta("GET", "/paneles/<panel_id>", "leer")
def ver_panel(ctx, actor, params, cuerpo, consulta):
    return 200, paneles.obtener(ctx.local, _entero(params["panel_id"]))
...
def cambiar_panel(ctx, actor, params, cuerpo, consulta):
    resultado = paneles.archivar(
        ctx.local, _entero(params["panel_id"]), cuerpo.get("estado", "archivado")
    )
    ctx.local.commit()
    return 200, resultado
...
def listar_miembros(ctx, actor, params, cuerpo, consulta):
    return 200, {
        "items": paneles.listar_miembros(
            ctx.local, _entero(params["panel_id"]), consulta.get("estado", "activo")
        )
    }
...
def agregar_miembros(ctx, actor, params, cuerpo, consulta):
    ...
    agregados = [paneles.agregar_miembro(ctx.local, panel_id, i) for i in ids]
    ctx.local.commit()
...
def quitar_miembro(ctx, actor, params, cuerpo, consulta):
    resultado = paneles.dar_de_baja_miembro(
        ctx.local, _entero(params["panel_id"]), params["id_persona"]
    )
    ctx.local.commit()
    return 200, resultado
...
def pendientes_cumplimiento(ctx, actor, params, cuerpo, consulta):
    return 200, {"items": bajas.pendientes_de_borrado_remoto(ctx.local)}


@ruta("POST", "/cumplimiento/reintentar", "cumplimiento")
def reintentar_cumplimiento(ctx, actor, params, cuerpo, consulta):
    resultado = bajas.reintentar_borrado_remoto(ctx.local, ctx.remoto)
    ctx.local.commit()
...
def crear_encuesta(ctx, actor, params, cuerpo, consulta):
    resultado = encuestas.crear(
        ctx.local, _entero(cuerpo.get("panel_id")), cuerpo.get("nombre"),
...
def listar_encuestas(ctx, actor, params, cuerpo, consulta):
    return 200, {"items": encuestas.listar(ctx.local, _entero(consulta.get("panel_id")))}


@ruta("GET", "/encuestas/<encuesta_id>", "leer")
def ver_encuesta(ctx, actor, params, cuerpo, consulta):
    return 200, encuestas.obtener(ctx.local, _entero(params["encuesta_id"]))
...
def cambiar_encuesta(ctx, actor, params, cuerpo, consulta):
    resultado = encuestas.cambiar_estado(
        ctx.local, _entero(params["encuesta_id"]), cuerpo.get("estado")
    )
    ctx.local.commit()
    return 200, resultado
...
def ver_participacion(ctx, actor, params, cuerpo, consulta):
    return 200, {
        "items": encuestas.listar_participacion(ctx.local, _entero(params["encuesta_id"]))
    }
...
def verificar_cruce(ctx, actor, params, cuerpo, consulta):
    return 200, encuestas.verificar_cruce(
        ctx.local, ctx.remoto, _entero(params["encuesta_id"])
    )
...
def auditoria_pii(ctx, actor, params, cuerpo, consulta):
    """Auditoría en vivo: columnas de PII en el store remoto. Debe dar 0."""
    hallazgos = remoto.auditar_columnas(ctx.remoto)
