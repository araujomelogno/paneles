/* Cumplimiento — el invariante de privacidad, visible.
   ...
   Tres cosas: la auditoría de que el store remoto no tiene columnas de PII,
   las bajas cuyo borrado remoto quedó pendiente de confirmar, y el detalle
...
export async function render(main, ctx) {
  ...
  main.innerHTML = encabezado('Cumplimiento', 'y privacidad',
    ...
    <div class="grid-2">
      <div class="stack">
        <div class="card">
          <div class="card-header"><span class="card-header-title">Auditoría del store remoto</span>
        ...
        <div class="card">
          <div class="card-header"><span class="card-header-title">Bajas con borrado remoto pendiente</span>
      ...
      <div class="stack">
        <div class="card">
          ...
          <div class="card-body stack">
            <div class="aviso">
              <h4>Uso semántico</h4>
              <p>Se borran los embeddings del store remoto. La persona sigue en el panel y
            ...
            <div class="aviso grave">
              <h4>Baja total</h4>
              <p>Cascada completa: se borra la PII de la bóveda local, se borran los
              embeddings del remoto y sale de todos los paneles.</p>
              <p>Queda una lápida con el token opaco —que no es PII— para poder probar
              que el retiro se atendió. El borrado local no espera al remoto: si el remoto
        ...
        <div class="card">
          ...
          <div class="card-body">
            <p class="small">La PII vive solo en el store local. Al remoto viaja únicamente
            ...
            remoto se audita contra una lista de campos identificatorios, y cada escritura
            remota pasa por una validación que aborta la operación si detecta una clave de PII.</p>
          </div>
        </div>
      </div>
    </div>`;
  ...
  try {
    ...
    contenedor.innerHTML = resultado.limpio
      ? `<div class="alert alert-success">
           0 columnas de PII en el store remoto. El esquema está limpio.
         </div>
         <p class="small muted">Se revisan todas las tablas del esquema remoto contra la
         lista de campos identificatorios de la bóveda y sus sinónimos habituales.</p>`
      : `<div class="alert alert-error">
           Se encontraron ${resultado.hallazgos.length} columna(s) de PII en el store remoto.
         </div>
...
async function cargarPendientes() {
  ...
  contenedor.innerHTML = `
    <div class="table-wrap"><table>
      <thead><tr><th>Identificador</th><th>Motivo</th><th>Baja local</th><th>Error</th></tr></thead>
      <tbody>${items.map((b) => `
        <tr>
          ...
          <td class="small td-muted">${esc(b.remoto_error || 'sin confirmar')}</td>
...
async function reintentar() {
  try {
    ...
    toast(errores
      ...
      : `${resultados.length} baja(s) confirmadas del lado remoto.`,
      errores ? 'err' : 'ok');
